...
const updateSceneStatus = (scene, status) => {
  ...
  if (mapType === 'Gaode') {
    ...
    if ((0, _lodash.isNumber)(zoom)) map?.setZoom(zoom + 1, true);
    if ((0, _lodash.isNumber)(rotation)) map?.setRotation(360 - rotation, true);
    if ((0, _lodash.isNumber)(pitch)) map?.setPitch(pitch, true);
  } else {
    if ((0, _lodash.isNumber)(zoom)) scene?.setZoom(zoom);
    if (center) scene?.setCenter(center);
    if ((0, _lodash.isNumber)(rotation)) scene.setRotation(rotation);
    if ((0, _lodash.isNumber)(pitch)) scene?.setPitch(pitch);
  }
};
 ...
 */
function syncScene(scenes, options) {
  const {
    zoomGap = 0,
    mainIndex = 0
  } = options ?? {};
