...
export class DataProvider extends EventEmitter {
    constructor() {
         ...
         */
        this.getOriginData = createSelector(this.getSourceData, this.getSourceParser, (data, parser) => {
            return transformSource({ data, parser });
        });
        /**
         * 获取聚合配置
         */
        this.getClusterOptions = createSelector(this.getClusterType, this.getEnableCluster, this.getExtent, this.getNodeSize, this.getRadius, this.getMinZoom, this.getMaxZoom, this.getZoomStep, function (clusterType, enableCluster, clusterExtent, clusterNodeSize, clusterRadius, minZoom, maxZoom, clusterZoomStep) {
            return {
                clusterType,
                enableCluster,
                clusterExtent,
         ...
         * 获取各个层级下的聚合点数组
         */
        this.getClusterLevels = createSelector(this.getOriginData, this.getClusterOptions, ({ locations }, clusterOptions) => {
            return clusterLocations(locations, clusterOptions);
        });
        /**
         * 获取聚合点的检索器
         */
        this.getClusterIndex = createSelector(this.getClusterLevels, (clusterLevels) => {
            return buildIndex(clusterLevels);
        });
        /**
         * 获取当前需要展示的聚合点
         */
        this.getViewLocations = createSelector(this.getClusterIndex, this.getMapZoom, this.getMapBounds, (clusterIndex, zoom, bounds) => {
            return clusterIndex.getMapLocations(zoom, bounds);
        });
        this.getLocationWeightRange = createSelector(this.getClusterIndex, this.getMapZoom, function (clusterIndex, zoom) {
         ...
         * 获取当前层级下的聚合线数据
         */
        this.getAggregatedFlows = createSelector(this.getClusterIndex, this.getOriginData, this.getMapZoom, (clusterIndex, { flows: originFlows }, zoom) => {
            return clusterFlows(originFlows, clusterIndex, zoom);
        });
        /**
         * 获取当前需要展示的聚合线数据
         */
        this.getFilterFlows = createSelector(this.getViewLocations, this.getAggregatedFlows, this.getMaxTopFlowNum, (filterLocations, fullFlows, maxTopFlowNum) => {
        ...
        this.getFilterLocations = createSelector(this.getViewLocations, this.getFilterFlows, (locations, flows) => {
         ...
         */
        this.getFlowWeightRange = createSelector(this.getAggregatedFlows, function (flows) {
            if (!flows.length) {
                return [0, 0];
            }
         ...
         */
        this.getFilterFlowWeightRange = createSelector(this.getFilterFlows, function (flows) {
            if (!flows.length) {
                return [0, 0];
            }
            const { weight: minWeight } = flows[0];
