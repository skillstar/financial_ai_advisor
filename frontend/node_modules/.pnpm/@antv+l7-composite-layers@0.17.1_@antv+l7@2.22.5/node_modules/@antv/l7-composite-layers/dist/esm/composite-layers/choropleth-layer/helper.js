 ...
 * 是否开启自定义标注图层坐标字段
 */
export const isLabelPosition = (position) => {
    if (isUndefined(position) || isBoolean(position))
        return false;
    if (isUndefined(position['coordinates']) &&
 ...
 */
export const parserLabeSourceData = (source, labelOptions) => {
 ...
 * 获取标注图层配置项
 */
export const getLabelLayerOptions = (options, source) => {
    return Object.assign(Object.assign({}, getLabelOptions(options)), { source: parserLabeSourceData(source, options.label) });
};
