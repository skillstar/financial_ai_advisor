...
class LayerGroup extends event_emitter_1.default {
    ...
    addTo(scene) {
        ...
        for (const layer of this.layerMap.values()) {
            layer.once('inited', (e) => {
                layerIndex++;
                this.emit(constants_1.LayerGroupEvent.INITED_LAYER, e);
                if (layerIndex === layerLength) {
                    this.emit(constants_1.LayerGroupEvent.INITED_LAYERS);
                }
            });
            layer.addTo(scene);
        }
    ...
    addLayer(layer) {
        ...
        if (this.scene) {
            layer.once('inited', (e) => this.emit(constants_1.LayerGroupEvent.INITED_LAYER, e));
