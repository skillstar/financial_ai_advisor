...
exports.clusterByHCA = clusterByHCA;
const tslib_1 = require("tslib");
const kdbush_1 = tslib_1.__importDefault(require("kdbush"));
...
function createKDTree(clusters, nodeSize) {
    return new kdbush_1.default(clusters, getX, getY, nodeSize, 
...
function createCluster(x, y, childIds, weight, id) {
    return {
        id,
        lng: (0, transform_1.xLng)(x),
        lat: (0, transform_1.yLat)(y),
        x,
        y,
        zoom: Infinity,
        weight,
...
function getNextClusters(points, zoom, tree, { clusterRadius, clusterExtent }) {
    ...
    for (let i = 0; i < points.length; i++) {
        ...
        let wx = p.x * weight;
        let wy = p.y * weight;
        const id = (0, utils_1.getLocationId)();
 ...
 * @returns
 */
function clusterByHCA(locations, state) {
    var _a, _b;
    const { minZoom, maxZoom, clusterZoomStep, clusterNodeSize, enableCluster } = state;
    const trees = [];
    let clusters = locations
        .map((location) => (Object.assign(Object.assign({}, location), { x: (0, transform_1.lngX)(location.lng), y: (0, transform_1.latY)(location.lat), weight: location.weight, zoom: Infinity, childIds: [] })))
